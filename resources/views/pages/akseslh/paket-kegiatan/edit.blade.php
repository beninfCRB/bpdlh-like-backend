...
@section('title', 'Edit Data Paket Kegiatan')
                        ...
                        <div class="row">
                                ...
                                <select class="form-control" required id="master_sub_tematik_kegiatan_id"
                                    ...
                                    @isset($masterSubTematikKegiatan)
                                        @foreach ($masterSubTematikKegiatan as $item)
                                            @if (old('master_sub_tematik_kegiatan_id', $data->master_sub_tematik_kegiatan_id) == $item['id'])
                                                <option class='form-control' value="{{ $item['id'] }}" selected>
                                                    {{ $item['tematik'] }}
                                                </option>
                                            @else
                            ...
                            <div class="form-group @error('nama_paket_kegiatan') has-error @enderror col-md-12">
                                <label for="nama_paket_kegiatan">Nama Paket Kegiatan <span
                                        class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="nama_paket_kegiatan"
                                    name="nama_paket_kegiatan" placeholder="Nama Paket Kegiatan"
                            ...
                            <div class="form-group @error('tahap_pencairan_paket_kegiatan') has-error @enderror col-md-3">
                                ...
                                <input type="number" min=1 max=5 class="form-control" id="tahap_pencairan_paket_kegiatan"
                                    ...
                                    value="{{ old('tahap_pencairan_paket_kegiatan', $data->tahap_pencairan_paket_kegiatan) }}"
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody id="dynamicForm-tbody">
                                            @foreach ($data->tahap_salur_paket_kegiatan as $item)
                                                <tr>
                                                    <td width='50%'>Tahapan Salur ke-{{ $loop->iteration }}</td>
                                                    <td width='50%'>
                                                        <div class="input-group">
                                                            <input type="number" id="example-input2-group1"
                                                                name="porsi_pencairan[{{ $loop->iteration }}]"
                                                                class="form-control" min="1"
                                                                value="{{ $item->porsi_pencairan }}" required>
                                            ...
                                            @isset($masterKomponenRab)
                                                @foreach ($masterKomponenRab as $item)
                                                    <tr>
                                                        <td>
                                                            <input type="checkbox"
                                                                name="komponen_rab[{{ $loop->iteration }}][id]"
                                                                id="" value="{{ $item['id'] }}"
                                                                {{ $data->standar_rab_paket_kegiatan->where('master_komponen_rab_id', $item['id'])->first() ? 'checked' : null }}>
                                                        </td>
                                                        ...
                                                        <td>
                                                            <input type="number" class="form-control"
                                                                ...
                                                                value="{{ $data->standar_rab_paket_kegiatan->where('master_komponen_rab_id', $item['id'])->first()
                                                                    ? $data->standar_rab_paket_kegiatan->where('master_komponen_rab_id', $item['id'])->first()->standar_harga_unit
                                                                    : null }}">
                                                        </td>
                                                    </tr>
                                                @endforeach
