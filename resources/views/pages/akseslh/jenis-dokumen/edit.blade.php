        ...
        <div class="col-md-6">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <form role="form" action="{{ route('jenis-dokumen.update', $data->id) }}" method="POST"
                            ...
                            <select class="form-control" required id="tahapan_pengajuan_kegiatan_id"
                                ...
                                @isset($tahapanPengajuanKegiatan)
                                    @foreach ($tahapanPengajuanKegiatan as $item)
                                        @if (old('tahapan_pengajuan_kegiatan_id', $data->tahapan_pengajuan_kegiatan_id) == $item['id'])
                        ...
                        <div class="form-group @error('jenis_dokumen') has-error @enderror">
                            ...
                            <input type="text" class="form-control" id="jenis_dokumen" name="jenis_dokumen"
                                placeholder="Jenis dokumen" value="{{ old('jenis_dokumen', $data->jenis_dokumen) }}">
                            @error('jenis_dokumen')
                                {{ $message }}
                            @enderror
                        </div>
