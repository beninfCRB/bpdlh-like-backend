    ...
    <div class="col-md-6">
      <div class="panel panel-primary">
        ...
        <div class="panel-body">
          <form role="form" action="{{ route('master-data-bank.update', $data->id) }}" method="POST">
            @method('PUT')
            @csrf
            <div class="form-group @error('nama-bank') has-error @enderror">
              <label for="nama-bank">Nama Bank <span class="text-danger">*</span></label>
              <input type="text" class="form-control" id="nama-bank" name="nama_bank"
                value="{{ old('nama-bank', $data->nama_bank) }}">
              @error('nama-bank')
                {{ $message }}
              @enderror
            </div>
            <div class="form-group @error('kode-bank') has-error @enderror">
              <label for="kode-bank">Kode Bank <span class="text-danger">*</span></label>
              <input type="text" class="form-control" id="kode-bank" name="kode_bank"
                value="{{ old('kode-bank', $data->bank_code) }}">
              @error('kode-bank')
                {{ $message }}
              @enderror
            </div>
