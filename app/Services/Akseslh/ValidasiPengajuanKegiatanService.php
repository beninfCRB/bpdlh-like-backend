...
class ValidasiPengajuanKegiatanService extends AppService implements AppServiceInterface
{
    ...
    public function apiGetBydId($id)
    {
        ...
        if (auth()->user()->role_user == 'maker') {
            $result = [
                ...
                'document'                  => $model->document()->whereNotIn('group', ['surat_permintaan_nomor_rekening', 'dokumen_pendukung'])->get(),
    ...
    public function update_termin_1_tanpa_pencairan_termin_2($id, $data)
    {
        $read   =   $this->model->newQuery()->find($id);
