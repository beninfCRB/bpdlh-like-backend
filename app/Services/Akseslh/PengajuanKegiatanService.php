...
class PengajuanKegiatanService extends AppService implements AppServiceInterface
{
    ...
    public function createTemp($data)
    {
        ...
        try {
            ...
            // Menyiapkan response untuk review pengajuan
            $review_pengajuan_kegiatan = [
                'tematik_kegiatan_id'               => $newData->paket_kegiatan->master_sub_tematik_kegiatan->tematik_kegiatan->id,
                'tematik_kegiatan'                  => $newData->paket_kegiatan->master_sub_tematik_kegiatan->tematik_kegiatan->tematik_kegiatan,
                'sub_tematik_tematik_kegiatan_id'   => $newData->paket_kegiatan->master_sub_tematik_kegiatan->sub_tematik_kegiatan->id,
    ...
    public function revisi_pengajuan_kegiatan_update($id, $data)
    {
        ...
        try {
            ...
            if ($total == (int)$model->caping_rab) {
                ...
                \DB::rollBack();
                return $this->sendError(null, 'Nilai RAB harus sama dengan nilai yang sudah ditentukan', 422);
            }
        } catch (\Exception $exception) {
