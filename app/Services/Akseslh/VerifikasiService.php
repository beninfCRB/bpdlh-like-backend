  ...
  public function update($id, $data)
  {
    ...
    try {
      ...
      if ($data['status'] == 0) {
        ...
        if ($data['paket_kegiatan_id'] == $read->paket_kegiatan_id) {
          ...
          foreach ($dataLogTahapanPengajuanKegiatan as $d2) {
            if ($d2->tahapan_pengajuan_kegiatan->deskripsi_kegiatan == "Pengajuan") {
              $dataLogTanggalMulai = $d2->tanggal_masuk;
              $dataLogTanggalSelesai = $d2->tanggal_selesai;
            }
          }
          // Menghapus data dengan query builder langsung
          $this->modelLogTahapanPengajuanKegiatan->newQuery()
            ->where('pengajuan_kegiatan_id', $id)
            ->delete();
          ...
          foreach ($dataTahapanPengajuanKegiatan as $dt) {
            $this->modelLogTahapanPengajuanKegiatan->newQuery()->create([
              ...
              'tanggal_masuk'                 => ($dt->deskripsi_kegiatan == "Pengajuan" ? $dataLogTanggalMulai : date("Y-m-d")),
              'tanggal_selesai'               => ($dt->deskripsi_kegiatan == "Pengajuan" ? $dataLogTanggalSelesai : date("Y-m-d"))
            ]);
          }
