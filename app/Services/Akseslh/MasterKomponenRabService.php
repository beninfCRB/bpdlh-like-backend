...
use App\Models\MasterKomponenRab;
use App\Services\AppService;
use App\Services\AppServiceInterface;
use Illuminate\Database\Eloquent\Model;
use Yajra\DataTables\Facades\DataTables;

class MasterKomponenRabService extends AppService implements AppServiceInterface
{

    public function __construct(MasterKomponenRab $model)
    {
    ...
    {
        $model = $this->model->query()->with(['jenis_komponen', 'satuan'])->orderBy('created_at', 'DESC');
    ...
    public function getAllAttr()
    {
        ...
        $result->transform(function ($items, $key) {
            return [
                'id'                    => $items->id,
                'komponen_rab'          => $items->komponen_rab,
                'standar_harga_unit'    => $items->standar_harga_unit,
            ];
        });
    ...
    public function create($data)
    {
        ...
        try {
            ...
            $data = $this->model->newQuery()->create([
                'jenis_komponen_rab_id'    =>  $data['jenis_komponen_rab'],
                'satuan_id'                =>  $data['satuan'],
                'komponen_rab'      =>  $data['master_komponen_rab'],
                'standar_harga_unit'       =>  $data['standar_harga_unit'],
            ]);
    ...
    public function update($id, $data)
    {
        ...
        try {
            ...
            $read->jenis_komponen_rab_id    =   $data['jenis_komponen_rab'];
            $read->satuan_id                =   $data['satuan'];
            $read->komponen_rab      =   $data['master_komponen_rab'];
            $read->standar_harga_unit       =   $data['standar_harga_unit'];
            $read->save();
